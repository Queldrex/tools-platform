    ...
    <div className="min-h-screen" style={{ background: '#070b14' }}>
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <span className="text-[11px] font-bold uppercase tracking-widest px-3 py-1 rounded-full border" style={{ color: '#f87171', borderColor: 'rgba(248,113,113,0.3)', background: 'rgba(248,113,113,0.08)' }}>No AI Â· Pure Regex</span>
          <span className="text-sm font-bold text-white/30">3 free scans/day Â· Unlimited with Pro</span>
        </div>
        <h1 className="text-4xl font-black text-white mb-3">Prompt Injection <span style={{ color: '#f87171' }}>Detector</span></h1>
        <p className="text-white/55 text-base mb-6 max-w-xl">Test your AI application against 15 deterministic attack patterns â€” instruction overrides, DAN jailbreaks, token smuggling, encoding attacks, and more. Zero AI hallucination: pure regex, instant results.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="md:col-span-2 rounded-xl border p-5" style={{ background: '#0d1117', borderColor: 'rgba(255,255,255,0.07)' }}>
            ...
            <p className="text-sm text-white/60 leading-relaxed">Scans any text for prompt injection patterns using 15 deterministic regex patterns â€” no AI, no false positives. Detects jailbreaks (DAN, STAN), instruction override attempts, data extraction probes, role-play attacks, and encoded bypass techniques. Results are instant and fully explainable.</p>
          </div>
        ...
        <div className="rounded-2xl border p-6 mb-6" style={{ background: '#0d1117', borderColor: 'rgba(255,255,255,0.08)' }}>
          ...
          <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Paste user input you want to test for injection attacksâ€¦" rows={8}
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => analyze()} disabled={loading || !prompt.trim()}
                ...
                {loading ? 'Scanningâ€¦' : 'Scan for Injections'}
              </button>
            </div>
          </div>
        </div>
        ...
        {paywall && !loading && (
          <div className="rounded-2xl border p-8 text-center mb-6" style={{ background: 'rgba(248,113,113,0.05)', borderColor: 'rgba(248,113,113,0.2)' }}>
            ...
            <p className="text-white/50 text-sm mb-6 max-w-sm mx-auto">Pro gives you unlimited prompt injection testing and access to all security tools â€” $79/month.</p>
