...
function BreachLookupContent() {
  ...
  async function handlePasswordCheck(e: React.FormEvent) {
    e.preventDefault()
    if (!password) return
    setPwLoading(true); setPwResult(null); setPwError('')
    try {
  ...
  async function runDomainScan(domainArg?: string) {
    const d = (domainArg ?? domain).trim()
    if (!d) return
    if (!domainArg && hasUsedFreeScan) { setShowDomainPaywall(true); return }
    setDomainLoading(true); setDomainResult(null); setDomainError(''); setShowDomainPaywall(false)
    try {
      const clean = d.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0].trim()
      ...
      router.replace(`?tab=domain&q=${encodeURIComponent(clean)}`, { scroll: false })
    } catch (err) {
  ...
  async function handleDomainScan(e: React.FormEvent) {
    e.preventDefault()
    if (hasUsedFreeScan) { setShowDomainPaywall(true); return }
    await runDomainScan()
  }
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      e.preventDefault()
      if (tab === 'password') {
        if (!password || pwLoading) return
        setPwLoading(true); setPwResult(null); setPwError('')
        checkPassword(password)
          .then(r => setPwResult(r))
          .catch(err => setPwError(err instanceof Error ? err.message : 'Check failed'))
  ...
  }, [tab, password, pwLoading, domain, domainLoading, hasUsedFreeScan])
      ...
      <main className="max-w-2xl mx-auto px-6 py-10">
          ...
          <div>
            <form onSubmit={handlePasswordCheck} className="space-y-4">
              ...
              <button type="submit" disabled={!password || pwLoading}
                ...
                {pwLoading ? 'Checking…' : <>Check Password <span className="opacity-30 text-xs">⌘↵</span></>}
              </button>
            </form>
          ...
          <div>
            <form onSubmit={handleDomainScan} className="space-y-4">
              ...
              <button type="submit" disabled={!domain || domainLoading}
                className="w-full py-3 rounded-xl text-sm font-black text-black transition-all hover:scale-[1.01] disabled:opacity-40 disabled:cursor-not-allowed"
                style={{ background: 'linear-gradient(135deg,#06d6ff,#0891b2)' }}>
                {domainLoading ? 'Scanning…' : <>Scan Domain <span className="opacity-30 text-xs">⌘↵</span></>}
              </button>
            </form>
            ...
            {domainResult && (
              <div className="mt-6 space-y-3">
                <div className="rounded-xl border p-5 flex items-center justify-between flex-wrap gap-3" style={{ background: '#0d1117', borderColor: 'rgba(255,255,255,0.08)' }}>
                  <div>
        ...
        <div className="mt-14 border-t pt-10" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              { n: '01', title: 'Password tab (k-anonymity)', body: 'SHA-1 hash computed in browser. Only the first 5 characters are sent to Have I Been Pwned\'s API. Your full password never leaves your device.' },
              { n: '02', title: 'Domain tab (live security audit)', body: 'Our server fetches HTTP headers for your domain and queries DNS records. Checks HTTPS, HSTS, X-Frame-Options, CSP, X-Content-Type, DMARC, and SPF.' },
            ].map(s => (
...
export default function BreachLookupPage() {
  return <Suspense fallback={null}><BreachLookupContent /></Suspense>
}
