...
interface Result { title: string; executiveSummary: string; sections: Section[]; fullText: string; yourCompany: string; clientCompany: string; hasAccess: boolean }
    ...
    <div className="min-h-screen" style={{ background: '#070b14' }}>
      ...
      <main className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="flex gap-3 flex-wrap mb-6">
          <BuyToolButton toolId="proposal-generator" price={29} className="inline-flex items-center gap-1 text-sm font-black px-4 py-2 rounded-xl text-black" style={{ background: 'linear-gradient(135deg,#6366f1,#4f46e5)' }} />
          ...
          <div className="rounded-2xl border p-6" style={{ background: '#0d1117', borderColor: 'rgba(255,255,255,0.08)' }}>
            ...
            {result.hasAccess ? (
              ...
              <div>
                ...
                {/* Show first 2 sections fully */}
                {result.sections?.slice(0, 2).map((s, i) => (
                  <div key={i} className="border-t pt-5 mb-4" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                    <p className="text-xs font-black uppercase tracking-widest mb-3" style={{ color: '#4ade80' }}>{s.heading}</p>
                    <div className="text-sm text-white/65 leading-relaxed whitespace-pre-line">{s.content}</div>
                  </div>
                ))}
                {/* Blur remaining sections */}
                <div className="relative mt-2">
                  <div className="select-none pointer-events-none" style={{ filter: 'blur(6px)', opacity: 0.4, maxHeight: 280, overflow: 'hidden' }}>
                    {result.sections?.slice(2).map((s, i) => (
                      <div key={i} className="border-t pt-5 mb-4" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                        <p className="text-xs font-black uppercase tracking-widest mb-3" style={{ color: '#4ade80' }}>{s.heading}</p>
                        <div className="text-sm text-white/65 leading-relaxed whitespace-pre-line">{s.content}</div>
                      </div>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center px-4">
                    <div className="w-full max-w-md">
                      <PaywallCard toolId="proposal-generator" toolName="Proposal Generator" oneTimePrice={29} freeLimit={1} accent="#4ade80" />
                    </div>
                  </div>
        ...
        <div className="mt-14 rounded-2xl border p-6 text-center" style={{ background: 'rgba(99,102,241,0.05)', borderColor: 'rgba(99,102,241,0.15)' }}>
          ...
          <div className="flex gap-3 justify-center flex-wrap">
            <BuyToolButton toolId="proposal-generator" price={29} className="px-5 py-2.5 rounded-xl text-sm font-black text-black" style={{ background: 'linear-gradient(135deg,#6366f1,#4f46e5)' }} />
